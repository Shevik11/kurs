...
export class tables {
    ...
    async showTablesOfSeason(season_name) {
        ...
        return { result, columnWidths }; // Повертаємо об'єкт з результатом та ширинами колонок
        ...
        // Отримуємо елемент, в який будемо вставляти результати
        // const resultElement = document.getElementById('result');
        // Очищаємо вміст елементу
        // resultElement.innerHTML = "";
        // Форматуємо заголовок
        // const header = Object.keys(columnWidths).map(column => column.padEnd(columnWidths[column])).join('\t');
        // // Створюємо та додаємо заголовок
        // const headerParagraph = document.createElement('p');
        // headerParagraph.textContent = header;
        // resultElement.appendChild(headerParagraph);
        // result.forEach(item => {
        //     // Форматуємо рядок
        //     const row = Object.keys(columnWidths).map(column => String(item[column]).padEnd(columnWidths[column])).join('\t  ');
        //     // Створюємо та додаємо рядок
